"""教員コンソールの**全ルートを固定する**（段階的な立て直し・段階 0-1）。

`docs/design/staged-refactor.md` の約束 1 と 5: 移動は振る舞いを変えない。
`manage.py` を画面ごとに分ける（段階 4）と、ルートの登録は何十か所にも散る。
...
ルートを意図して足した・消したときは、写しを作り直す:
    ...
    AIJUDGE_UPDATE_ROUTES=1 uv run pytest apps/reviewconsole/tests/test_reviewconsole_routes.py
...
from aijudge_reviewconsole import Console, create_app
from aijudge_submission import FilesystemArtifactStore

SNAPSHOT = Path(__file__).with_name("routes_reviewconsole.txt")
...
def test_the_routes_are_exactly_the_snapshot(tmp_path: Path) -> None:
    ...
    lines = route_lines(create_app(console))
    database.dispose()
