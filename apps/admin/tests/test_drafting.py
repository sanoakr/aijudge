...
def test_a_draft_becomes_an_ordinary_task_spec() -> None:
    ...
    version = build_task_version(
        result.spec, course_id=COURSE, subject_profile="cs_lang_c_intro", authored_by=AUTHOR
    )
...
def test_a_course_without_an_outline_gets_no_section() -> None:
    ...
    assert (
        _course_section(
            Blueprint(knowledge_components=("cs.loops",), subject_profile="cs_lang_c_intro")
        )
        == ""
    )
