...
def _skeleton(database: Database):
    ...
    register_kc(database, key="cs.loops.control", label="制御", namespaces=SPACES, seeding=True)
    return area
...
def test_a_namespace_the_profile_did_not_declare_is_refused(database: Database) -> None:
    ...
    with pytest.raises(AdminError) as exc:
        register_kc(database, key="csci.loops", label="ループ", namespaces=SPACES, seeding=True)
    assert "名前空間" in str(exc.value)


def test_the_declared_namespace_is_accepted(database: Database) -> None:
    kc = _skeleton(database)
...
def test_a_child_needs_a_unit_that_the_skeleton_declares(database: Database) -> None:
    """骨格に無い単位の下には足せない。**「先に追加してください」とは言わない** ──
    分野も単位も画面からは足せないので、そう案内すると二度断られる。"""
    with pytest.raises(AdminError) as exc:
        register_kc(
            database, key="cs.loops.control.termination", label="停止条件", namespaces=SPACES
        )
    assert "骨格" in str(exc.value)
    assert "先に" not in str(exc.value)


def test_a_child_is_accepted_once_the_unit_exists(database: Database) -> None:
    _skeleton(database)
    parent = list_for_namespaces(database, SPACES)[0]
    parent = next(
        kc for kc in list_for_namespaces(database, SPACES) if kc.key == "cs.loops.control"
    )
    child = register_kc(
        database, key="cs.loops.control.termination", label="停止条件", namespaces=SPACES
    )
    assert child.parent_id == parent.id
    assert child.parent_key == "cs.loops.control"
...
def test_registering_the_same_key_twice_does_not_duplicate(database: Database) -> None:
    first = _skeleton(database)
    second = register_kc(
        database, key="cs.loops", label="別の名前", namespaces=SPACES, seeding=True
    )
    assert first.id == second.id
    # 骨格の分野と単位で 2 件。同じキーを二度入れても増えない。
    assert len(list_for_namespaces(database, SPACES)) == 2
...
def test_a_malformed_key_is_refused(database: Database) -> None:
    ...
    for key in ("loops", "cs..loops", "CS.loops", "cs.1loops", "cs.配列の走査", "情報.loops"):
        with pytest.raises(AdminError):
            register_kc(database, key=key, label="x", namespaces=SPACES, seeding=True)
...
def test_the_refusal_says_what_would_be_accepted(database: Database) -> None:
    ...
    """
    with pytest.raises(AdminError) as exc:
        register_kc(database, key="cs.配列", label="配列", namespaces=SPACES, seeding=True)
...
def test_retiring_keeps_the_component(database: Database) -> None:
    ...
    register_kc(database, key="cs.loops.control.terminaton", label="打ち間違い", namespaces=SPACES)

    retired = retire_kc(
        database,
        key="cs.loops.control.terminaton",
        superseded_by_key="cs.loops.control.termination",
    )
    ...
    assert "cs.loops.control.terminaton" in keys
    # 現役だけを求めれば出てこない。
    live = [kc.key for kc in list_for_namespaces(database, SPACES, include_deprecated=False)]
    assert "cs.loops.control.terminaton" not in live


def test_a_retired_component_cannot_be_a_successor(database: Database) -> None:
    """辿った先がまた引退している、を作らない。"""
    _skeleton(database)
    register_kc(database, key="cs.loops.control.a", label="a", namespaces=SPACES)
    register_kc(database, key="cs.loops.control.b", label="b", namespaces=SPACES)
    retire_kc(database, key="cs.loops.control.b")
    with pytest.raises(AdminError):
        retire_kc(database, key="cs.loops.control.a", superseded_by_key="cs.loops.control.b")
...
def test_an_unregistered_component_is_refused(database: Database) -> None:
    with pytest.raises(AdminError) as exc:
        assert_registered(database, ("cs.loops.control.termination",))
...
def test_a_task_cannot_name_an_unregistered_component(database: Database, course) -> None:
    ...
    spec = TaskSpec(
        key="ex01/p1",
        statement="## 課題 ##\n\n本文",
        knowledge_components=("cs.loops.control.terminaton",),
    )
    with pytest.raises(AdminError):
        save_task(
            database,
...
def test_a_task_with_registered_components_is_saved(database: Database, course) -> None:
    ...
    spec = TaskSpec(
        key="ex01/p1",
        statement="## 課題 ##\n\n本文",
        knowledge_components=("cs.loops.control.termination",),
    )
    saved = save_task(
        database,
        course_id=course.id,
...
def test_usage_counts_tasks_and_courses(database: Database, course) -> None:
    ...
    assert rows["cs.loops.control.termination"].tasks == 1
    assert rows["cs.loops.control.termination"].courses == 1
...
def test_an_unused_component_can_be_deleted(database: Database) -> None:
    ...
    役にも立たない行が永久に並ぶ。
    """
    register_kc(database, key="cs.c_langauge", label="打ち間違い", namespaces=SPACES, seeding=True)
...
def test_a_used_component_is_never_deleted(database: Database, course) -> None:
    ...
    with pytest.raises(AdminError, match="使われています"):
        delete_kc(database, key="cs.loops.control.termination")
    assert "cs.loops.control.termination" in [
        kc.key for kc in list_for_namespaces(database, SPACES)
    ]
...
def test_a_component_with_children_is_not_deleted(database: Database) -> None:
    ...
    # 子から先に消せば、親も消せる（単位 → 分野の順）。
    delete_kc(database, key="cs.loops.control.termination")
    delete_kc(database, key="cs.loops.control")
    delete_kc(database, key="cs.loops")
...
def test_a_retired_component_can_still_be_deleted_if_unused(database: Database) -> None:
    ...
    register_kc(database, key="cs.typo", label="打ち間違い", namespaces=SPACES, seeding=True)
...
def test_the_label_and_description_can_be_corrected(database: Database) -> None:
    ...
    表示の都合で持ち出すことになる。
    """
    kc = register_kc(database, key="cs.loops", label="ルーブ", namespaces=SPACES, seeding=True)
...
def test_editing_a_used_component_is_allowed(database: Database, course) -> None:
    ...
    updated = edit_kc(database, key="cs.loops.control.termination", label="停止条件")
...
def test_the_label_cannot_be_emptied(database: Database) -> None:
    ...
    register_kc(database, key="cs.loops", label="ループ", namespaces=SPACES, seeding=True)
...
def test_a_component_outside_the_course_selection_is_refused(database: Database) -> None:
    ...
    """
    _skeleton(database)
    register_kc(database, key="cs.python", label="Python", namespaces=SPACES, seeding=True)
