...
（`aijudge_core.task.Task`）。`TaskSpec` には公開と締切の欄しか無かったので
（提出開始・採点開始・受付終了は 2026-09-25 に足した）、版を上げるたびに
`Task` を作り直す `save_task` は、**欄の無い日程を既定（空）へ戻していた**
── 問題セットで揃えたあとに課題を 1 つ直すと、その課題
