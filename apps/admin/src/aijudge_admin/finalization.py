...
def sweep_deadlines(
    ...
    *,
    ...
    猶予はコースの `auto_finalize_after_hours`。None のコースは飛ばす
    （自動確定しないという設定であって、設定漏れではない ── 既定は None で、
    教員が明示的に入れて初めて自動確定が始まる）。締切の無い課題も飛ばす。
    ...
    **何度走らせても同じ結果になる。** 確定済みの採点は
    ...
    for course in _courses(database, course_id):
        if course.auto_finalize_after_hours is None:
            continue
        with database.unit_of_work() as uow:
            for task in uow.tasks.list_for_course(course.id):
                cutoff = deadline_for(task.due_at, course.auto_finalize_after_hours)
