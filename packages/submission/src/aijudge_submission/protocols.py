...
class AttentionCounts:
    """このコースで**人が動かないと先へ進まないもの**の件数（#189）。

    左の帯が全ページで出す数である。**参考の数え上げは持たない** ── 提出の
    総数や受講者数は「どこに用があるか」を教えないので、帯に数字として
    載せる価値がない（載せる分だけ全ページがクエリを払う）。

    `contested` は学習者が再確認を依頼して、まだ答えていないもの。
    `unfinalized` は最新の採点が確定していない提出で、**教員・TA 自身の
    試行は数えない**（#108）── 数えると自動確定が「まだ閉じていない成績が
    ある」と言い続ける。ここは一括確定の画面と同じ数でなければならない。
    """
...
class ReviewRepository(Protocol):
    ...
    **`Finalization` の存在が成績の確定を意味する**（`HumanReview` ではない）。
    ...
    def attention_counts_for_course(self, course_id: CourseId) -> AttentionCounts:
        ...
        **数だけを引く。** 左の帯は全ページに出るので、行を持ってきて
        `len()` を取ると 1 ページごとに提出の件数ぶんの転送を払う。
        一括確定の画面が使う `pending_counts` は課題ごとに 1 回引くので
        課題 41 問のコースでは 42 クエリになる ── 帯にそれは置けない。

        **一括確定の画面と同じ数を返すこと。** 帯の数字と、開いた先の
        件数が違うと、帯は数字を出す意味を失う（どちらが本当か分からない
        なら、確かめに開くことになる）。
        """
        ...
