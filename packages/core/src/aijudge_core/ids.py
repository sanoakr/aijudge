...
PREFIXES: dict[str, str] = {
    ...
    "dft": "TaskDraft",
    # 習熟度が動いた瞬間の記録（#328）。**習熟度そのものには ID が無い**
    # ── あれは学習者 × KC で 1 行の持ち替えで、こちらは追記される点である。
    "skp": "SkillPoint",
    # 中断から続けられるアップロードの受け皿（#119）。**提出ではない** ──
    ...
    "run": "RunRequest",
    # IDE の画面を 1 回開いてから閉じるまで（ADR 0023）。行動記録はこの単位で
    # 束ねる。ログインのセッション（`ses`）とは別物 ── 1 回のログインで IDE を
    # 何度も開き直すし、同じ IDE を 2 つの窓で開くこともある。
    "ide": "IdeSession",
}
