...
def bulk_finalizable(run: GradingRun, request: ReviewRequest | None) -> bool:
    ...
    操作なので、自動確定と同じ制限は課さない。

    外すのは 2 つ。

    - 未対応の異議申立があるもの ── そこは教員が 1 件ずつ読むべきものとして残す
    - **人が採点する観点があるもの**（`awaiting_human`）── 一括確定は
      「読まずに責任を取る」操作だが、この観点には**そもそも読むべき値が
      存在しない**。まとめて閉じれば、その観点の重みを除いた暫定の点が
      そのまま成績になる。責任を取る対象が無いのだから、責任では代われない
      （Issue #7）

    評価器が落ちた観点（`unscored_criteria`）は従来どおり通す。あちらには
    AI の判定こそ無いが、決定的評価の結果は出ており、教員は暫定の点を見て
    署名できる。人採点の観点は誰も何も出していない。
    """
