...
class GradingCompleted(_Event):
    ...
    # 成績にも測定にも数えない提出か（#108・#197）。
    #
    # **`Submission.is_trial` と同じ名前にする。** 「数えるかどうか」を訊く
    # 述語を 2 通り持つと、片方だけ直る日が来る ── 実際、判定は既にあるのに
    # 観測レコードと習熟度がそれを見ていなかった。
    #
    # **イベントに載せるのは、購読側が提出を引けないからである**（P6）。
    # S7 は `kc_outcomes` だけを見て習熟度を更新する設計で、採点の内部にも
    # 保存先にも触らない ── ここで「提出を引いて訊く」ことにすると、
    # その独立が崩れる。
    is_trial: bool = False
