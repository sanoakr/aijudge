...
class Blueprint(BaseModel):
    ...
    **KC を必ず持つ。** 何を問う課題か決めずに生成させると、既存の課題と
    ...
    # 教員から AI への指示。1 行 1 件、**任意**。
    #
    # **必須事項の列ではない。** 以前は「制約」という名前で「課題文に必ず
    # 書かせたいこと」だけを受けていたが、教員が渡したいものはそれより広い
    # ── 「標準入力から読むこと」のような外せない条件と並んで、「速度より
    # 読みやすさを重視した題材で」のような**方針の希望**がある。名前が狭いと、
    # 書ける欄があるのに書かれないまま生成が回る。
    #
    # 強さの違いは**書き方で教員が表す**（「必ず」「できれば」）。欄を 2 つに
    # 割らない ── どちらに書くかを迷わせるだけで、モデルへの渡り方は同じ。
