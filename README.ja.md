...
**利用ガイド**（学生・TA・教員向け、画面付き）: <https://sanoakr.github.io/aijudge/>
── 原稿は [`docs/guide/`](docs/guide/) にあります。
...
- **携帯でも同じことができます。** 幅の狭い画面では表が 1 件 1 枚に畳まれ、どの値が何なのかは見出しが各欄に付いて回ります ── 列を画面の外へ送ると見出しが失われ、見出しの無い値は情報になりません。
- **ブラウザで書いて出せます。** 問題セットによっては、ファイルの代わりにエディタで答えます。課題ごとのタブ、課題が受け付ける形式だけ（`.c`・`.py` はプログラム、`.md` はオンラインのレポート試験の文章）、採点と同じ sandbox での試しの実行、10 秒ごとの自動保存。受付が終わると、最後に保存した内容が保存した時刻で自動的に提出されます。
...
- **試験と追試に対応します。** 問題セットを公開まで TA に見せないことも、名簿で
  選んだ学習者にだけ出すこともできます。追試は学習者ごとの締切ではなく別の問題セット
  として作ります。名簿は画面・API・CLI のどれからでも登録できます（ADR 0025）。
- **オンラインのコーディングテスト。** 問題セットをブラウザのエディタで解く形に切り替えられます（補完の有無も選べます）。試しの実行は採点の sandbox を使いますが、採点ではありません。答えがどう書かれたか（打鍵・貼り付け・画面を離れた時間）は記録され、そのコースの教員だけが再生でき、見たことは監査ログに残ります。確かめる価値のある場所に印を付けますが、判定も減点もしません（ADR 0023・0024・0026）。
- **測定のためのブラインド採点**は、教員が選ぶのではなく**システムが標本抽出** します。選ばせると難しい提出ばかりが集まり、測った一致率が歪むためです。
- **画面の作りは学習者側と同じもの**です。行き先の帯は狭い画面では畳まれ、押せば同じ縦並びの一覧が開きます。片方で覚えた形が、もう片方でもそのまま使えます。
...
| 画像・PDF（スクリーンショット、手書き） | PDF は本文を取り出してレポートと同じように採点。画像は、**ローカルの** VLM が書き起こして課題の照合パターンで決定的に判定する（修了証のスクリーンショットなど・ADR 0021/0022）か、観点を「人が採点する」と宣言して教員が採点する |
...
提出ファイルは**ディスクにそのまま置きます**。S3 互換のストアへ差し替える口はありますが（`ObjectArtifactStore`）、いま配線されている実装はありません ── 動画のような大きな提出を受け始めたときに、初めて要る話です。
...
uv run aijudge-runner                       # ブラウザのエディタの試しの実行（採点ではない）
uv run aijudge-ide-close --once             # エディタの問題セットの受付終了時に自動保存を提出
```
...
ブラウザのエディタの作業の記録と自動保存も、同じ期間で、同じく人の手で消します。

```fish
uv run aijudge-admin activity purge         # 下見
...
| ブラウザのエディタ（オンラインのコーディングテスト） | 動く。手元で 150 名の負荷試験済み、**運用機ではまだ測っていない** |

下 2 行を省かずに書いてあるのは意図的です。このコードベースは、根拠を示せない数字を出さず `NOT_MEASURED` と言う作法を持っており、**それは合格とは別**です。

### 1.0.0 が意味すること

**運用を始められる、という意味です。精度の保証ではありません。**

一巡が動き、サンドボックスの隔離は検証済みで、本番の構成で提出から確定までを
通しで確かめてあります。学期を回すのに足りないものは、いまのところ見つかって
いません。

**採点精度はまだ測れません。**測るには学期ぶんの提出と、教員が実際に読んだ
記録（`HumanReview`）が要ります ── どちらも運用を始めてからでないと貯まりません。
**測れないものを「測った」ことにはしない**ので、精度の門は運用開始後も
`NOT_MEASURED` を返し続けます。数字が出るのは、根拠が貯まってからです。

つまり 1.0.0 は「**ここから実データを取り始める**」という区切りであって、
「精度が保証された」という区切りではありません。
...
| **手書きと OCR** | 手書き答案を撮影して提出。ローカルの VLM が書き起こし、**学習者が確認・修正**し、確定したテキストが提出物になる（書き起こし自体は、照合パターンで判定する画像について稼働済み・ADR 0021。残っているのは学習者が確認・修正する流れ） | 学習者の修正率（文字単位）≤ 10%、確定テキストでの採点精度がテキスト直接入力に対して κ の低下 ≤ 0.05 |
