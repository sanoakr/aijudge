...
- ステータス: **検討中**（`feat/online-coding-test` ブランチ。運用中の main には入れない）
- 日付: 2026-09-24
- 関連: 設計原則 P2, P3, P5, P7, P8 / ADR 0006, 0011, 0016, 0018, 0019, 0020 /
...
- I8: `app.py` の関門を関数に抜き出す。振る舞いは変えない純粋な抜き出しで、
  既存の提出テストがそのまま守りになる
- 出題先と秘匿の判定は `docs/design/task-visibility.md` 側の変更で、本書では足さない
...
IDE とは独立した問題として `docs/design/task-visibility.md`
（`feat/task-visibility` ブランチ）で先に決める。**本書はそれを前提にする。**
IDE 画面・実行・提出・行動記録の受け口は、すべて同じ判定（`may_see`）を通す。
...
- **保存期間**: **動画と同じ**（ADR 0020。2026-09-24 決定）。締切のある課題は締切から
  6 か月、締切の無い課題はセッションの開始から 1 年。過ぎたら `aijudge-admin` の
  purge で消す（`video purge` と同じ型。既定は下見、`--apply` で実行）
