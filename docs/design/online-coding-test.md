...
- ステータス: **段階 4 まで実装し、v1.18.0 で main に入れた（2026-09-24）。** 既存の課題には何も起きない（既定はファイル提出）。runner の本数と、IDE を実際に使えるようにする設定は、運用機で測ってから（§12）
- 日付: 2026-09-24
...
- 画面・カメラの録画。ブラウザからは他のアプリの画面が見えず、得られるものに
  ...
  スナップショットを撮る（§6.4）。**改訂（2026-09-26）**: 試験の課題に限り、
  許可を得た画面全体の静止画を撮る（ADR 0027）。カメラと動画はやらないまま
...
**配置を改めた（2026-09-24）**: 見出し（問題セット名・残り時間）とタブは上に固定する。
その下に問題文（高さを抑えて中で送る）。問題文の下は、左がエディタ、右が公開サンプル・
標準入力・出力・「実行」「提出」（同じ大きさ）。枠を超える長い行は折り返して表示する
（表示だけで、中身に改行は入らない）。エディタの配色は画面の昼／夜に合わせる。
...
**バックアップ（2026-09-24 決定）**: 本番が壊れても過去 6 ヶ月のどの時点にも戻せる
ように、オフボックスの restic は月次を 6 本残す（以前は 12 本）。行動記録の本体
（`/work/aijudge/activity`）もバックアップに入れる（動画は大きさを検討中で入れていない）。
したがって学習者のデータは、消したあともバックアップに最大 6 ヶ月残る ── 告知はこの
合計で書く（作業の記録は締切から最大 12 ヶ月、自動保存は受付終了から最大 7 ヶ月）。
...
**実装（2026-09-24、試用のあとに改めた）**:

- 入口は左の帯の「作業の記録」（教員だけ）。コース全体の一覧は**索引だけ**で作り、
  受講者全員を並べる（提出していない学習者にも辿れる）。提出の一覧と確認画面からも移れる
- 学習者ごとの画面は**問題ごと**に分ける（1 回のセッションで複数の問題を行き来する
  ので、セッションの合計では「この問題をどう書いたか」が読めない）。`tab` を持たない
  出来事（離席）は、そのとき開いていた問題に振り分ける。その問題で手を動かさなかった
  回は並べない
- 再生は**時間で進む**（10 秒を超える空白は 3 秒に詰める）。速さは 1・4・16 倍。表示は
  Monaco を読み取り専用で使い、いま入った文字を強調する。学習者が開いていた問題に追従する。
  提出には「何回目・得点・採用」と確認画面へのリンクを添える
- 実行の出力は記録しない（結果は画面に出したら消す設計・§8）。残すかは未決
...
#### 手元での 150 名の負荷試験（2026-09-24 18:50〜）

構成: 手元の Mac。PostgreSQL と sandbox のコンテナは colima の VM（**CPU 4・メモリ 8 GB**、
runc）の中、web はワーカー 2 本。`tools/ide_load/load.py` で 150 名を模した。

| 条件 | runner | 実行の数 | 実行待ち p50 / p95 / 最大 | 行動記録 p95・取りこぼし | 自動保存・提出の失敗 |
|---|---|---|---|---|---|
| 平常（1 人平均 45 秒に 1 回）8 分 | 4 | 1401（2.9 回/秒） | 0.54 / 1.07 / 1.6 秒 | 35 ms・0 / 7014 | 0・0 |
| 平常（同上）8 分 | 2 | 1327（2.7 回/秒） | 0.56 / 1.07 / 1.6 秒 | 34 ms・0 / 7018 | 0・0 |
| 集中（1 人平均 10 秒に 1 回）2 分 | 2 | 567（3.8 回/秒） | 16.8 / 19.2 / 19.8 秒 | 38 ms・0 / 1040 | 0・0 |
| 集中（同上）2 分 | 4 | 727（5.1 回/秒） | 6.3 / 13.6 / 14.4 秒 | 47 ms・0 / 1291 | 0・0 |

読み取れること:

- **平常の負荷は runner 2 本で足りる**（設計書 §10.2 の見積もり 3.3 回/秒と同じ桁）。
  p50 が約 0.5 秒なのは、画面が結果を 0.5 秒ごとに問い合わせる間隔が下限になっているため
- **集中すると待ちが伸びるが、期限切れ（30 秒）にも失敗にもならなかった。** 実行 1 回は
  コンパイルと実行の 2 コンテナで約 0.5 秒。runner を 2 → 4 本にして処理量は 1.35 倍
  （2 倍にならないのは VM の CPU 4 つをコンテナの起動が取り合うため）
- **実行の山は、行動記録・自動保存・提出に波及しなかった**（p95 はどれも 70 ms 未満、
  取りこぼし 0）。キューとプロセスを分けた効果（ADR 0024）がそのまま出ている
- 行動記録は 4 回分（約 20 分 × 150 名）で 64 MB。§6.6 の見積もりと同じ桁

**これは手元の数値であり、K を決める根拠にはしない。** 運用機（16 スレッド・runsc）は
CPU が 4 倍あるので、同じ集中でも待ちはずっと短いはずだが、それは運用機で測って確かめる。
運用機での計測は、授業期間（9/24・9/25）が明けてから、授業の無い時間帯に行う
