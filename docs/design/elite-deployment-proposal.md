...
Google OIDC ログイン（#121、§9）は 2026-09-07 に本番で稼働確認済み・issue クローズ済み。
...
## 9. Google OIDC ログイン（#121・#124・#125、2026-09-07 完了）

**本番で稼働確認済み（2026-09-07）。** `v0.28.1`（OIDC アダプタ・ログイン
画面切替え）・`v0.28.2`（ログインボタンのレイアウト修正）とも autodeploy
timer で elite に反映済み。`AIJUDGE_OIDC_SECRET_KEY` の設定、Google Cloud
Console での OAuth クライアント発行、`/manage/oidc-settings` での設定を
elite 上で実施し、実際に大学アカウントでログイン → 学生 UI・教員コンソール
双方のセッションが張られることまで確認した。以下は実施した手順の記録。

### やったこと（elite 上、`sano` で）
...
5. **確認済み（2026-09-07）**:
   ...
   - 実際に大学アカウントでログインでき、学生 UI・教員コンソール双方の
     セッションが張られた（Cookie 共有は #103 のとおり変わらない）。
   - `/auth/callback` への実際のリダイレクト（`client_id`・`redirect_uri`・
     `scope=openid+email`・`state`/`nonce`）も応答ヘッダで確認済み。
