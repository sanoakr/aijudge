...
出てくるのは**候補**で、**ここからは登録されない。** 「この候補を使う」を押すと
その 1 件が「知識要素を追加する」の欄に入るので、キー・名前・説明を確かめて
（必要なら直して）から登録する。**キーは登録すると変えられない** ── KC の ID が
キーから決まり、使われたあとは削除もできないので、モデルの付けた文字列を素通しに
しない。規則は手で足すときと同じ経路を通る（名前空間・親の実在・第 1 階層は
管理者のみ）。親と子は**親から順に**足す。
