...
- **画像や PDF を出す問題**は、タブの中の「ファイルを選ぶ」で選んで「ファイルを提出」を押します。
  画像や PDF は**自動では保存も提出もされません**。選んだまま提出しないと、受付が終わったときに失われます
- **動画はエディタの画面からは提出できません**。動画を出す問題は、その問題の画面のファイル選択から提出します
...
試験によっては、エディタを開くと**「画面全体を共有して始める」**が出ます。押して、共有する画面として
**画面全体**を選んでください（タブやウィンドウだけでは受験できません）。

- 共有している間、画面全体の**静止画**を 5〜15 秒おきと、外から大きく貼り付けたときの前後、エディタの画面を
  離れた直後に撮ります。動画・音声・カメラは撮りません
- 見られるのは**担当教員だけ**で、締切から 6 か月で消します。自動で減点したり、成績の計算に使ったりはしません
- 通知やほかのアプリも写ります。**試験の前に、関係のないアプリや通知を閉じてください**
- **共有を止めると提出できなくなります**。画面の上の「画面の共有を再開する」を押せば、また提出できます。
  書いた内容は自動で保存され、受付終了時の自動提出も行われます
- スマートフォンやタブレットのブラウザは画面の共有に対応していません。PC の Chrome・Edge・Firefox で受けてください
- この試験では、課題の画面からのファイルでの提出はできません（エディタから提出します）

!!! note "受付終了のときに自動で提出されます"
    受付が終わった時点で、**最後に保存された内容が自動で提出**されます（まだ出していない問題と、
    出したあとに書き換えた問題）。**画像・PDF は自動では提出されません**（選んだら「ファイルを提出」を押す）。提出時刻は最後に保存された時刻なので、それが締切より前なら
...
**配点**が決まっている課題では、提出ごとの得点が「**16 / 20 点（80%）**」のように点数と割合で出ます。
コースの画面の課題の一覧には各問題の配点が、問題セットの見出しには**合計点**が出ます。

- 問題セットに**クリア点**が決まっていれば、合計がクリア点に届くと見出しに **クリア** の印が付きます。
  届いていなければ「クリアまであと N 点」と出ます
- 確定していない点（AI の判定のまま・採点中）を含むうちは「（暫定）」と添えられます。
  担当教員の確認で点が変わると、合計とクリアも変わります
- 配点が決まっていない課題は、これまでどおり割合（%）だけが出ます
...
!!! note "課題が訂正されたとき"
    担当教員が問題文や配点を訂正すると、課題の「版」が上がります。**それまでの提出は消えません。**
    一覧に「**訂正前の版への提出**」と印が付いて残り、回数も数えられたままです（回数は課題の中の
    通し番号で、版が変わっても 1 回目には戻りません）。出し直す必要があるかは、担当教員の案内に従ってください。
...
画面の上の「**自動・昼・夜**」で、画面の明るさを切り替えられます（自動は PC やスマホの設定に合わせます）。
...
C のプログラム（最大値を求める）と、100 字ほどの感想（AI が判定）の 2 問があり、どちらも
ブラウザのエディタでも、ファイルでも提出できます。
