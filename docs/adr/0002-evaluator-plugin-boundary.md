...
- **プロファイルの `timeout_seconds` は「評価器 1 回の呼び出しの予算」であって、
  評価器の内部の上限ではない。** 評価器が自分の中で複数回実行する場合
  （テストランナーはケースごとに 1 回走る）、その上限は
  `evaluator_options` で別に宣言する。

  実測（2026-08-28）で踏んだ。AI 評価器のために `timeout_seconds` を 120 秒へ
  上げた結果、テストランナーが同じ値を 1 ケースあたりの上限として使い、
  無限ループの提出 1 件でワーカーが 10 分占有された。**1 つの設定が意味の違う
  2 つの上限を兼ねていた**のが原因で、値の選び方ではなく設定の設計の誤り。
  `evaluator_options.code_test_runner.case_timeout_seconds` に分けた。
